"""
金融詐騙預測模組：載入交易數據，生成特徵，並使用訓練好的模型預測詐騙風險。
"""
import os
import warnings
...
# 常量設定
PREDICT_DATA = os.environ.get('PREDICT_DATA_PATH')
...
def prepare_features(df):
    ...
    print("\n載入特徵生成器...")
    try:
        ensure_directory_exists('models/feature_generator.joblib')
...
def predict_fraud(model_path, features, account_numbers):
    """使用模型進行預測"""
    print("\n開始預測...")
    ...
    # 保存結果
    output_file = 'dataset/Output/account_risk_assessment.csv'
    ensure_directory_exists(output_file)
    account_risk.to_csv(output_file)
    print(f"\n完整風險評估結果已保存至 '{output_file}'")
...
def main():
    """主函數"""
    # 載入數據
    df = load_data(PREDICT_DATA)  # 使用環境變數

    # 準備特徵
    features, account_numbers = prepare_features(df)

    # 預測並分析結果
    predict_fraud('models/fraud_detection_model.joblib', features, account_numbers)
